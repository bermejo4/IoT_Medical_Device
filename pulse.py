...
def ad8232():
    ecg_pin_output=machine.ADC(28)
    lo_plus=machine.Pin(21, machine.Pin.IN, machine.Pin.PULL_DOWN)
    lo_minus=machine.Pin(20, machine.Pin.IN, machine.Pin.PULL_DOWN)
    conversion_factor = 3.3 / (65535)
    while True:
        if lo_plus==1 or lo_minus==1:
            print("!")
        else:
            signal_value_ecg = ecg_pin_output.read_u16() * conversion_factor
